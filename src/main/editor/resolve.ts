 ...
 * and `shell: true` is exactly what launch.ts must never do — the URI it passes
 * carries a hex-encoded path originating from a container label. Treating a
 * `.cmd` as a valid resolution just moves the failure from here to the launch,
 * where it surfaces as a bare "spawn EINVAL".
 */
 ...
 * through to the well-known `.exe` locations below.
 */
