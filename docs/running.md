...
| Cursor opens its **Agents** window instead of the editor     | A Cursor setting, not boxwarden. Recent versions open the Agents window on startup: **Cursor Settings → Agents → Open Agents Window on startup**, turned off.                                                                                                                            |
| Cursor opens a window that never attaches                    | Cursor's `dev-container` authority is the hex of a JSON blob, not of a folder path like VS Code's; boxwarden builds the right one per editor. It also needs the `devcontainer.config_file` label, and refuses with a reason naming VS Code when a container carries only the folder one. |
| Cursor: `spawn podman ENOENT` while setting up the container | Cursor got the URI right and is now running its own container CLI. It shells out to whatever its dockerPath setting names, and that name has to be resolvable on Cursor's PATH — an absolute path to `podman.exe` rather than a bare `podman`. Nothing to do with boxwarden.             |
