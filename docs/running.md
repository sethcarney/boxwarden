...
| Cursor opens a window that never attaches, or errors         | Cursor's `dev-container` authority is a hex-encoded JSON blob, not a hex-encoded folder path like VS Code's — boxwarden builds the right one per editor since this was found. It also needs the `devcontainer.config_file` label, and its Dev Containers extension shells out to `docker`, so a Podman-only machine needs a docker-compatible CLI on PATH. |
| Cursor opens its **Agents** window instead of the editor     | A Cursor setting, not boxwarden. Recent versions open the Agents window on startup: **Cursor Settings → Agents → Open Agents Window on startup**, turned off. boxwarden hands Cursor the same `--folder-uri` it hands VS Code, and the setup page shows exactly which binary it launched.                                                                  |
